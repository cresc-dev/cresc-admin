...
// installing/restarting are in-flight; failed is only an after-the-fact hint
...
export function InstancesPanel({ target }: { target: ServiceStatusTarget }) {
  ...
  // Instance state is node-local (file registry), so ask that node's own API
  ...
  // Update/rollback is node-level: one global install, then server/worker
  // restart together
  const nodeVersion =
    rows.find((row) => row.role === 'server')?.version ?? rows[0]?.version;

  // Node-level deploy status shows directly on the update button:
  // in-flight first, then the most recent failure
