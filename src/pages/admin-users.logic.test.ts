...
describe('table config', () => {
  ...
  test('tier options cover every tier including custom', () => {
    expect(tierOptions.map((option) => option.value)).toEqual([
      'free',
      'standard',
      'premium',
      'pro',
    ...
    expect(tierLabelMap.get('ultra')).toBe('Ultra');
    expect(tierLabelMap.get('unknown')).toBeUndefined();
    expect(expiryShortcutDays).toEqual([7, 30, 365]);
  });
});

describe('statusMeta', () => {
  test('maps status to badge, colour class and label', () => {
