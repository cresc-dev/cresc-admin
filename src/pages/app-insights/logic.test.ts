...
describe('summarizeTraffic', () => {
  ...
  it('folds the window into totals, shares and rankings', () => {
    ...
    // The daily average covers completed days only (including the zero-request one)
    ...
    // Device counts cannot be summed across days; the peak single day is used
...
describe('funnel', () => {
  ...
  it('adds served totals, coverage and the deleted flag per version', () => {
    ...
    // The cumulative rate uses two distinct-device counts, so it is window
    // independent and cannot structurally exceed 100%
