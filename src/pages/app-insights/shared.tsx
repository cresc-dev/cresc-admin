...
/** The server answers 503 when the analytics store is down; retrying that is
 * pointless, every other error keeps the default retry policy. */
...
// Today's entry keeps growing server-side; catching up once a minute while
// the page is open is enough.
...
/** In-place notice for a failed read: a 503 carries the server's
...
/** One stat tile: label → big number → a one-line hint. */
...
export interface BarListItem {
  ...
  count: number;
  percent: number;
  /** Gray bar: catch-all rows such as unknown or other. */
  muted?: boolean;
  /** Red bar: rows that need attention, such as refusals or failures. */
  alert?: boolean;
  title?: string;
}

/** Ranked bar list: the region panel's layout, reused for any "name → count". */
...
/** A bundle: its version name (grayed out once deleted) plus a short hash
 * that copies the full value. */
