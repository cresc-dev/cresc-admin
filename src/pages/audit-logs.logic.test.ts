...
describe('buildAuditCsvRow / AUDIT_CSV_HEADER', () => {
  const actionMap = getActionMap(t);

  test('header has ten columns ending with API Key', () => {
    expect(AUDIT_CSV_HEADER).toHaveLength(10);
    expect(AUDIT_CSV_HEADER[0]).toBe('Time');
    expect(AUDIT_CSV_HEADER[9]).toBe('API Key');
  });
