...
function McpConnectionsPage() {
  ...
  // The endpoint must follow the same base URL the requests use (custom
  // endpoint / self-hosting); resolveApiBaseUrl already prefers the custom
  // endpoint, it is in the key only so a switch re-resolves
  ...
  // Failure toasts come from the MutationCache default; the modal stays open on failure so the user can fix and retry
