...
let handleResponse: typeof import('./response')['handleResponse'];
let RequestError: typeof import('./response')['RequestError'];

beforeAll(async () => {
  ({ handleResponse, RequestError } = await import('./response'));
});
...
describe('handleResponse', () => {
  ...
  it('throws RequestError and calls logout on 401 instead of resolving undefined', async () => {
    ...
    expect((caught as RequestErrorInstance).status).toBe(401);
  ...
  it('throws RequestError with server message on a 4xx business error', async () => {
    ...
    expect(caught).toBeInstanceOf(RequestError);
    expect((caught as RequestErrorInstance).message).toBe('bad params');
  ...
  it('surfaces the status when the server returns a non-JSON error page', async () => {
    ...
    expect(caught).toBeInstanceOf(RequestError);
    expect((caught as RequestErrorInstance).status).toBe(502);
    expect((caught as RequestErrorInstance).message).toContain('502');
  });
});
