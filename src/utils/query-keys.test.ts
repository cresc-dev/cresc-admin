...
describe('domain key factories', () => {
  ...
  describe('serviceStatusKeys', () => {
    test('node keys share the node prefix', () => {
      const prefix = serviceStatusKeys.node();
      expect(prefix).toEqual(['serviceStatus', 'node']);
      for (const key of [
        serviceStatusKeys.metrics(),
        serviceStatusKeys.api5xxEvents(20),
      ]) {
        // one invalidate on this prefix refreshes both node queries
        expect(key.slice(0, prefix.length)).toEqual(prefix);
      }
      expect(serviceStatusKeys.api5xxEvents(20)).toEqual([
        'serviceStatus',
        'node',
        'api5xxEvents',
        20,
    ...
    test('cloud run keys share the cloudRun prefix', () => {
      const prefix = serviceStatusKeys.cloudRun();
      for (const key of [
        serviceStatusKeys.cloudRunStatus(),
        serviceStatusKeys.cloudRunMetrics(),
        serviceStatusKeys.cloudRunRevisions('api'),
        serviceStatusKeys.cloudRunImages(),
      ]) {
        expect(key.slice(0, prefix.length)).toEqual(prefix);
      }
      expect(serviceStatusKeys.cloudRunRevisions(null)).toEqual([
        'serviceStatus',
        'cloudRun',
        'revisions',
        null,
    ...
    test('global panels do not match the node or cloud run prefix', () => {
      const globals = [
    ...
    test('all() is a prefix of every service status key', () => {
      ...
      expect(serviceStatusKeys.cloudRunImages()[0]).toBe('serviceStatus');
    });
  });
