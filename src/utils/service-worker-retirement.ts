export const LEGACY_CRESC_CACHE_PREFIX = 'cresc-admin-';
...
/**
 * Identify the retired root-level Cresc worker without touching registrations
 * owned by another application that happens to share the same origin.
 */
export function isLegacyCrescRegistration(
      ...
      serviceWorker
        ...
        .then((registrations) =>
          Promise.allSettled(
            registrations
              .filter((registration) =>
                isLegacyCrescRegistration(registration, pageOrigin),
              )
    ...
    cleanupTasks.push(
      cacheStorage
        ...
        .then((keys) =>
          Promise.allSettled(
            keys
              .filter((key) => key.startsWith(LEGACY_CRESC_CACHE_PREFIX))
